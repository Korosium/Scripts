"use strict";
/**
 * JavaScript implementation of the XChaCha20-Poly1305 algorithm.
 * 
 * The XChaCha20-Poly1305 construction is an extended 192-bit nonce variant 
 * of the ChaCha20-Poly1305 construction, using XChaCha20 instead of ChaCha20. 
 * When choosing nonces at random, the XChaCha20-Poly1305 construction allows 
 * for better security than the original construction. The draft attempt to standardize 
 * the construction expired in July 2020. (from Wikipedia)
 * 
 * @link   https://en.wikipedia.org/wiki/XChaCha20-Poly1305
 * @link   https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-xchacha-03
 * @file   This file defines the xchacha20_poly1305 global constant.
 * @author Korosium
 */
const xchacha20_poly1305 = (() => {
    ...
    /**
     * The length in bytes of the counter in the Block-H function.
     */
    const BLOCK_H_COUNTER_LENGTH = 4;

    /**
     * The length of the Block-H nonce length in bytes.
     */
    const BLOCK_H_NONCE_LENGTH = 16;
    ...
    const xchacha20 = (() => {
        ...
        return {
            ...
            /**
             * Create the sub-key stream to obfuscate the real key.
             * 
             * @param {number[]} key   The key used for the encryption or the decryption process.
             * @param {number[]} nonce The nonce used for the encryption or the decryption process.
             * 
             * @returns {number[]} The sub-key stream.
             */
            block_h(key, nonce) {
                let state = init(key, to_uint_32(nonce.slice(0, BLOCK_H_COUNTER_LENGTH)), nonce.slice(BLOCK_H_COUNTER_LENGTH, BLOCK_H_NONCE_LENGTH));
                for (let i = 0; i < ROUNDS / 2; i++) state = inner_block(state);
                const stream = serialize(state);
                return stream.slice(0, 16).concat(stream.slice(48, 64));
            },
            ...
            encrypt(key, counter, nonce, plaintext) {
                const sub_key = this.block_h(key, nonce.slice(0, BLOCK_H_NONCE_LENGTH));
                const chacha20_nonce = [0, 0, 0, 0].concat(nonce.slice(BLOCK_H_NONCE_LENGTH, NONCE_LENGTH));
                let ciphertext = [];
                for (let i = 0; i < plaintext.length; i += STATE_SIZE) {
                    const stream = this.block(sub_key, counter + i / STATE_SIZE, chacha20_nonce);
    ...
    const poly1305 = (() => {
        ...
        return {
             ...
             */
            key_gen(key, nonce) {
                return xchacha20.block(key, 0, nonce).slice(0, KEY_LENGTH);
            },
    ...
    const combined = (() => {
        ...
        return {
            ...
            /**
             * Encrypt some data with the XChaCha20-Poly1305 algorithm.
             * 
            ...
            encrypt(key, plaintext, nonce, aad) {
                ...
                const sub_key = primitive.xchacha20.block_h(params.key, params.nonce.slice(0, BLOCK_H_NONCE_LENGTH));
                const chacha20_nonce = [0, 0, 0, 0].concat(params.nonce.slice(BLOCK_H_NONCE_LENGTH, NONCE_LENGTH));
                const one_time_key = primitive.poly1305.key_gen(sub_key, chacha20_nonce);
                const ciphertext = primitive.xchacha20.encrypt(params.key, 1, params.nonce, params.data);
            ...
            /**
             * Decrypt some data with the XChaCha20-Poly1305 algorithm.
             * 
            ...
            decrypt(key, ciphertext, aad) {
                ...
                if (params.tag === tag) {
                    return primitive.xchacha20.encrypt(params.key, 1, params.nonce, params.data);
                }
                else {
    ...
    const decrypt = (() => {
        ...
        const array = (() => {
            ...
            return {
                ...
                /**
                 * Decrypt a ciphertext byte array with the XChaCha20-Poly1305 algorithm.
                 * 
                 * @param {number[] | string | Uint8Array | ArrayBuffer}             key              The key used for the decryption process.
        ...
        const hex = (() => {
            ...
            return {
                ...
                /**
                 * Decrypt a ciphertext hex string with the XChaCha20-Poly1305 algorithm.
                 * 
                 * @param {number[] | string | Uint8Array | ArrayBuffer}             key              The key used for the decryption process.
        ...
        const base64 = (() => {
            ...
            return {
                ...
                /**
                 * Decrypt a ciphertext Base64 string with the XChaCha20-Poly1305 algorithm.
                 * 
                 * @param {number[] | string | Uint8Array | ArrayBuffer}             key              The key used for the decryption process.
                 * @param {string}                                                   ciphertext       The ciphertext to decrypt.
                 * @param {number[] | string | Uint8Array | ArrayBuffer | undefined} [aad = number[]] The additional authenticated data, if needed.
    ...
    const utility = (() => {
        ...
        return {
            ...
            generate_nonce() {
                ...
                return primitive.xchacha20.block(key, counter, nonce).slice(0, NONCE_LENGTH);
            }
